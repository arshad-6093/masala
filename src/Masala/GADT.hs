...
data KnownOp where KnownOp :: (KnownNat a,KnownNat b,KnownNat c) => Cmd a b c -> KnownOp

--instance  (KnownNat a,KnownNat b,KnownNat c)=> Show (Cmd a b c) where
--    show c =

pickValue :: KnownOp -> Integer
pickValue (KnownOp op) = pickV op

pickV :: forall a b c . KnownNat a => Cmd a b c -> Integer
pickV _ = natVal (Proxy :: Proxy a)
